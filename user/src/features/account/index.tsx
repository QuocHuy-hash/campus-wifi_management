...
const getPolicyBandwidthText = (policy: UserPolicy | null): string => {
  if (!policy) return i18n.t("account.unlimited");
  ...
  if (!download) return i18n.t("account.unlimited");
...
const getPolicySessionHoursText = (policy: UserPolicy | null): string => {
  ...
  if (!sessionSeconds) return i18n.t("account.unlimited");
...
const getPolicyDeviceLimitText = (policy: UserPolicy | null): string => {
  ...
  if (!maxDevices) return i18n.t("account.unlimited");
  return i18n.t("account.deviceLimit", { count: maxDevices });
};
...
export default function Account() {
  ...
  const handleUpdateProfile = async () => {
    ...
    if (normalizedFullName.length < 2 || normalizedFullName.length > 100) {
      setProfileFormError(t("account.fullNameError"));
      return;
    }
    if (
      normalizedPhone &&
      ...
      setProfileFormError(t("account.phoneInvalid"));
      return;
    }
  ...
  const handleChangePassword = () => {
    setPasswordError("");
    if (!currentPassword) {
      setPasswordError(t("password.enterPassword"));
      return;
    }
    ...
    if (newPassword !== confirmPassword) {
      setPasswordError(t("common.confirmPasswordNotMatch"));
      return;
    }
    dispatch(
      changePassword({
      ...
      >
        {loading && !user && (
          <Card className="p-8 text-center border-border">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-sm text-muted-foreground">{t("account.loadingProfile")}</p>
            </div>
          </Card>
        )}
          ...
          <Card className="mb-4 p-4 border border-red-200 bg-red-50">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-red-800">{t("account.loadError")}</p>
          ...
          <div className="p-4 space-y-4">
            <div>
              <p className="text-xs font-medium text-card-foreground mb-2 flex items-center gap-1.5">
                <User size={12} /> {t("account.personalInfo")}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg">
                  <Shield size={16} className="text-muted-foreground" />
                  <div>
                    <p className="text-[10px] text-muted-foreground">{t("account.role")}</p>
                ...
                {user?.groups && user.groups.length > 0 && (
                  <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg sm:col-span-2">
                    ...
                    <div>
                      <p className="text-[10px] text-muted-foreground">{t("account.group")}</p>
            ...
            <div className="pt-4 border-t border-border">
              <p className="text-xs font-medium text-card-foreground mb-2 flex items-center gap-1.5">
                <Globe size={12} /> {t("account.linkedAccounts")}
              </p>
                ...
                {user?.linkedProviders && user.linkedProviders.length > 0 ? (
                  user.linkedProviders.map((provider: any, idx: number) => (
                    ...
                    >
                      ...
                      >
                        {provider.isActive ? t("account.providerActive") : t("account.providerInactive")}
                      </span>
                    </div>
                  ))
                ) : (
        ...
        <Card className="mb-4 p-4 border-border">
          <p className="text-xs font-medium text-card-foreground mb-3 flex items-center gap-1.5">
            <Gauge size={12} /> {t("account.policy", { name: primaryPolicy?.name || primaryRole })}
          </p>
          ...
          {getPolicyAuthType(primaryPolicy) && (
            <div className="mt-3 pt-3 border-t border-border">
              <p className="text-[10px] text-muted-foreground mb-1">{t("account.authType")}</p>
        ...
        <Card className="mb-4 p-4 border-border">
          <p className="text-xs font-medium text-card-foreground mb-3 flex items-center gap-1.5">
            <Package size={12} /> {t("account.usageStats")}
          </p>
          <div className="mb-4">
            <p className="text-[10px] text-muted-foreground mb-2">{t("account.today")}</p>
            <div className="grid grid-cols-3 gap-2">
              <div className="p-2 bg-blue-50 dark:bg-blue-950/30 rounded border border-blue-100 dark:border-blue-900 text-center">
                ...
                <p className="text-xs font-semibold text-blue-600 dark:text-blue-400">
                  {dailyUsage ? formatBytes(dailyUsage.totalDownloadBytes) : t("common.loading")}
                </p>
              </div>
              <div className="p-2 bg-green-50 dark:bg-green-950/30 rounded border border-green-100 dark:border-green-900 text-center">
                ...
                <p className="text-xs font-semibold text-green-600 dark:text-green-400">
                  {dailyUsage ? formatBytes(dailyUsage.totalUploadBytes) : t("common.loading")}
                </p>
              </div>
              <div className="p-2 bg-indigo-50 dark:bg-indigo-950/30 rounded border border-indigo-100 dark:border-indigo-900 text-center">
                ...
                <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                  {dailyUsage ? formatBytes(dailyUsage.totalBytes) : t("common.loading")}
                </p>
              </div>
            </div>
          </div>
        ...
        <Card className="p-4 border-border">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-medium text-card-foreground flex items-center gap-1.5">
              <Laptop size={12} /> {t("account.approvedDevices")}
            </p>
            <span className="text-[10px] text-muted-foreground">
              {primaryPolicy
                ? t("account.maxDevices", { value: getPolicyDeviceLimitText(primaryPolicy) })
                : t("account.noDeviceLimit")}
            </span>
          </div>
          {devicesLoading ? (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-muted-foreground">{t("account.loadingDevices")}</p>
            </div>
          ) : userDevices.length === 0 ? (
            <div className="text-center py-6">
              <Wifi size={24} className="mx-auto text-muted-foreground mb-2" />
              <p className="text-xs text-muted-foreground">{t("account.noApprovedDevices")}</p>
            </div>
          ) : (
            <div className="space-y-2">
              {userDevices.map((device: UserDevice) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        title={device.isOnline ? t("account.deviceOnline") : "Offline"}
                      />
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    >
                      {device.status === 'ACTIVE' ? t("account.deviceActive") : device.status === 'BLOCKED' ? t("account.deviceBlocked") : t("account.deviceInactive")}
                    </span>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {changePasswordSuccess ? (
            <div className="py-8 text-center">
              ...
              <p className="text-lg font-medium text-card-foreground">{t("account.changePasswordSuccess")}</p>
              <p className="text-sm text-muted-foreground mt-1">
                {t("account.changePasswordSuccessDesc")}
              </p>
            </div>
          ) : (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="current-password" className="text-sm">
                  {t("password.currentPasswordLabel")}
                </Label>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={t("password.newPasswordMinPlaceholder")}
                    value={newPassword}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="confirm-password" className="text-sm">
                  {t("password.confirmNewPasswordLabel")}
                </Label>
                <div className="relative">
                  <Input
                    id="confirm-password"
