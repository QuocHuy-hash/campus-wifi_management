...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Improved has_form: specifically looking for form tags or input tags
        has_form = bool(re.search(r'<form|<input|<select|<textarea|type=["\'](?:password|email|text|number|tel|url|search|date|datetime-local|month|week|time|color|range|checkbox|radio)["\']', content, re.IGNORECASE))
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'legacy'}]
            for file in files:
